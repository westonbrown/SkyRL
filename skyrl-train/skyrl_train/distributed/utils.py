...
def init_custom_process_group(
    ...
    # NOTE: The pg_options parameter was renamed to backend_options in PyTorch 2.6.0.
    # Do not use string comparison for versions (e.g. "2.10" < "2.6" lexicographically).
    version_tokens = str(torch.__version__).split("+", 1)[0].split(".")
    try:
        major = int(version_tokens[0])
        minor = int(version_tokens[1])
    except (ValueError, IndexError):
        major, minor = (0, 0)
    pg_options_param_name = "backend_options" if (major, minor) >= (2, 6) else "pg_options"
